...
tickets_abertos = (df['des_status'] == 'Abertos').sum()

print(f"Quantidade de tickets abertos: {tickets_abertos}")
...
tickets_concluídos = (df['des_status'] == 'concluídos').sum()

print(f"Quantidade de tickets concluídos: {tickets_concluídos}")
...
tickets_cancelados = (df['des_status'] == 'cancelados').sum()

print(f"Quantidade de tickets cancelados: {tickets_cancelados}")
...
import pandas as pd
tipoVeiculo = "carros"
ap1 = f"https://brasilapi.com.br/api/fipe/veiculos/v1/{tipoVeiculo}"
resposta = requests.get(ap1)
dados = response.json()
...
fipe = df[df["nome"].str.contains("Fipe", case=False, na=False)]
print(fipe[["BYD", "nome"]])

resultado = fipe[fipe["BYD"].str.contains("BYD", case=False, na=False)]
print(resultado[["BYD", "nome"]])
...
mf = df[["roe", "dividend_yield"]].copy()
mf["rank_roe"] = mf["roc"].rank(ascending=False, method="min")
mf["rank_dividend_yield"] = mf["dividend_yield"].rank(ascending=False, method="min")
mf["magic_formula"] = mf["rank_roe"] + mf["rank_dividend_yield"]

carteira = mf.sort_values("magic_formula").head(0)

print(carteira[["roe", "dividend_yield", "magic_formula"]])
...
carteira = mf.sort_values("magic_formula").head(30)
...
df = pd.DataFrame(response.json())
varejo = df[df["setor"].str.contains("varejo", case=False, na=False)]
resultado = df.loc[['endividamento'.idxmax()], ["ticker", "setor", "preco", "endividamento"]]
...
IBGE = df[df["FNTSIGLA"].str.contains("IBGE", case=False, na=False)]

print(IBGE[["SERCODIGO", "FNTSIGLA", "SERNOME"]])

resultado = IBGE[IBGE["SERCODIGO"].str.contains("desemprego", case=False, na=False)]
...
CODIGO_ENCONTRADO = "ABATE12_ABQUBO12"
